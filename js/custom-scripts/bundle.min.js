// All Scripts From This Folder Bundled Together
...
function add_elements(t){t.preventDefault(),t.stopPropagation();var a=document.getElementsByClassName("pyramid_nav_subdropdown");if("block"!=a[0].style.display)for(i in a)a[i].style.display="block";else for(i in a)a[i].style.display="none"}!function(t){"use strict";t('a.js-scroll-trigger[href*="#"]:not([href="#"])').click(function(){if(location.pathname.replace(/^\//,"")==this.pathname.replace(/^\//,"")&&location.hostname==this.hostname){var a=t(this.hash);if((a=a.length?a:t("[name="+this.hash.slice(1)+"]")).length)return t("html, body").animate({scrollTop:a.offset().top-54},1e3,"easeInOutExpo"),!1}}),t(".js-scroll-trigger").click(function(){t(".navbar-collapse").collapse("hide")}),t("body").scrollspy({target:"#mainNav",offset:56});var a=function(){t("#mainNav").offset().top>100?t("#mainNav").addClass("navbar-shrink"):t("#mainNav").removeClass("navbar-shrink")};a(),t(window).scroll(a)}(jQuery);var navbar_buttons={main:{id:"navbarResponsive",status:!1},pyramid:{id:"pyramid_navbar_mob",status:!1}};function show_navbar(t){if(1==navbar_buttons[t].status)return navbar_buttons[t].status=!1,void(document.getElementById(navbar_buttons[t].id).style.display="none");for(i in navbar_buttons)i!=t&&0!=navbar_buttons[i].status&&(navbar_buttons[i].status=!1,document.getElementById(navbar_buttons[i].id).style.display="none");navbar_buttons[t].status=!0,document.getElementById(navbar_buttons[t].id).style.display="block"}
window.addEventListener("load",()=>{carousel.init("main_carousel","carousel_announce_wrap")&&(carousel.container.addEventListener("mousedown",carousel.onmousedown),carousel.container.addEventListener("touchstart",carousel.onmousedown))});const carousel={init:function(e,o){return carousel.container=document.getElementById(e),carousel.elems=carousel.container.getElementsByClassName(o),carousel.width=carousel.container.offsetWidth,carousel.length=carousel.elems.length,1!=carousel.length&&(carousel.mousedown=!1,carousel.mouseStart=0,carousel.offset={curr:0,left:-carousel.width,right:carousel.width,center:0,change_el_required:carousel.width/3},carousel.set_actives(0),carousel.indicators=carousel.add_indicators(),!0)},init_el:function(e){var o=carousel.elems[e];return o.style.display="block",{num:e,el:o}},add_indicators:function(){var e=document.createElement("div"),o=[];e.setAttribute("style","position: absolute; bottom: 2%; width: 100%; text-align: center;");var r={};for(i=0;i!=carousel.length;++i)r=document.createElement("span"),i==carousel.curr.num?r.setAttribute("style","background-color: white; border: 1px solid white; border-radius: 50%; height:10px; width:10px; margin: 0 2px; display: inline-block"):r.setAttribute("style","background-color: transparent; border: 1px solid white; border-radius: 50%; height:8px; width:8px; margin: 0 2px; display: inline-block"),r.id="carousel_indicator"+i,e.appendChild(r),o.push(r);return carousel.container.appendChild(e),o},set_actives:function(e){for(e<0?e=carousel.length-1:e>carousel.length-1&&(e=0),carousel.curr=carousel.init_el(e),0==e?(carousel.next=carousel.init_el(1),carousel.prev=carousel.init_el(carousel.length-1)):e==carousel.length-1?(carousel.next=carousel.init_el(0),carousel.prev=carousel.init_el(e-1)):(carousel.next=carousel.init_el(e+1),carousel.prev=carousel.init_el(e-1)),i=0;i!=carousel.length;++i)i!=carousel.prev.num&&i!=carousel.next.num&&i!=carousel.curr.num&&(carousel.elems[i].style.display="none");carousel.curr.el.style.left="0",carousel.next.el.style.left=carousel.offset.right+"px",carousel.prev.el.style.left=carousel.offset.left+"px"},onmousedown:function(e){carousel.mousedown=!0,void 0!==e.touches?carousel.mouseStart=e.touches[0].clientX:carousel.mouseStart=e.clientX,window.addEventListener("mouseup",carousel.onmouseup,{once:!0}),window.addEventListener("mousemove",carousel.onmousemove),window.addEventListener("touchend",carousel.onmouseup,{once:!0}),window.addEventListener("touchmove",carousel.onmousemove)},onmousemove:function(e){void 0!==e.touches?carousel.offset.curr=carousel.mouseStart-e.touches[0].clientX:carousel.offset.curr=carousel.mouseStart-e.clientX,carousel.offset.curr>0?carousel.move_el("next"):carousel.offset.curr<0&&carousel.move_el("prev")},onmouseup:function(){window.removeEventListener("mousemove",carousel.onmousemove),window.removeEventListener("touchmove",carousel.onmousemove),carousel.offset.curr>=carousel.offset.change_el_required?carousel.change_el("next"):carousel.offset.curr<=-carousel.offset.change_el_required?carousel.change_el("prev"):carousel.set_actives(carousel.curr.num),carousel.mousedown=!1,carousel.offset.curr=0},change_el:function(e){for("next"==e?++carousel.curr.num:--carousel.curr.num,carousel.set_actives(carousel.curr.num),i=0;i!=carousel.length;++i)i==carousel.curr.num?(carousel.indicators[i].style.height=carousel.indicators[i].style.width="10px",carousel.indicators[i].style.backgroundColor="white"):(carousel.indicators[i].style.height=carousel.indicators[i].style.width="8px",carousel.indicators[i].style.backgroundColor="transparent")},move_el:function(e){carousel.offset.curr<-carousel.width?carousel.offset.curr=-carousel.width:carousel.offset.curr>carousel.width?carousel.offset.curr=carousel.width:(carousel.curr.el.style.left=-carousel.offset.curr+"px",carousel.next.el.style.left=carousel.width-carousel.offset.curr+"px",carousel.prev.el.style.left=-(carousel.width+carousel.offset.curr)+"px")}};const refs=["vendor/fontawesome-free/css/all.min.css"];function loadCSSLazy(e){const t=document.getElementById("lazy-css");for(const s of e){const e=document.createElement("link");e.href=s;e.rel="stylesheet",e.type="text/css",t.appendChild(e)}}loadCSSLazy(refs);
